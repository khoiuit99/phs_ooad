...
export default function CreateImport() {
  ...
  const [khachhang, setKhachHang] = useState(1);
  const [thanhtien, setThanhTien] = useState(0);
  const [unTounch, setUnTouch] = useState(false);

  var listBookImport = JSON.parse(localStorage.getItem("listImportBook"));
  ...
  const onSubmit = async (data) => {
    ...
    if (window.confirm("Bạn có muốn tạo phiếu nhập?")) {
      setUnTouch(true);
      axios
        .post("http://localhost/phs_ttk/api/phieunhap", testData)
        .then((response) => {
          if (response.status === 200) {
            window.alert("Tạo phiếu thành công");
            localStorage.removeItem("listImportBook");
            setUnTouch(false);
            history.goBack();
          }
        })
  ...
  const getClient = () => {
    axios
      .get("http://localhost/phs_ttk/api/nxb")
                ...
                <div className="card-tools">
                  <div className="col-sm-12">
                    <ol className="breadcrumb float-sm-right">
                      ...
                      <li className="breadcrumb-item active">Tạo phiếu nhập</li>
                    </ol>
                  </div>
                </div>
              </div>
                    ...
                    <div class="row">
                      <div class="col-sm-12">
                        ...
                        >
                          ...
                          <tbody>
                            {listBookImport?.map((itemBook, key) => {
                              return (
                                <tr key={key}>
                                  <td>{key + 1}</td>
                                    ...
                                    <input
                                      ...
                                      onKeyDown={(e) => {
                                        setThanhTien(
                                          (presum) =>
                                            presum +
                                            e.target.value * itemBook.GiaNhap
                                        );
                                      }}
